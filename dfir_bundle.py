...
BUNDLE_DIR_NAME = "realistic_dfir_bundle"
...
def build_structure(base: Path):
    """
    Build a synthetic but realistic DFIR bundle.
    NOTE: Registry hives and EVTX here are TEXT placeholders,
    not real binaries. Use them to exercise your pipeline structure.
    Replace with real artifacts later if you want true parsing.
    """

    # --- 1. "Registry hives" (placeholders) -----------------------------
    # These are text files but named as real hives. Regipy will complain,
    # but the structure matches a real case layout.
    software = base / "SOFTWARE"
    system = base / "SYSTEM"
    sam = base / "SAM"
    security = base / "SECURITY"
    ntuser = base / "NTUSER.DAT"
    ...
    write_text(
        sam,
        """
        ; Synthetic SAM hive placeholder
        ; (no real credentials here, just placeholder)
        """,
    )
    ...
    write_text(
        security,
        """
        ; Synthetic SECURITY hive placeholder
        """,
    )
    ...
    # --- 2. Windows Event Logs (text, but named .evtx) ------------------
    logs_dir = base / "WindowsEventLogs"

    write_text(
        logs_dir / "Security.evtx",
        """
        <!-- Synthetic Security.evtx (this is NOT a real EVTX binary) -->
        <Event>
          <System>
...
def main():
    cwd = Path.cwd()
    base = cwd / BUNDLE_DIR_NAME

    if base.exists():
        print(f"[!] {base} already exists. Remove it first if you want a clean rebuild.")
    else:
